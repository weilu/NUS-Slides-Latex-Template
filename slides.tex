...
% - add speaker notes
% - go through improved top responsive algo proof
...
\begin{frame}{Background}
  How to divide people into groups?
  \begin{itemize}
    ...
    \begin{itemize}
      \item Mostly theoretical analysis
        \note{Past works study stabilitysolution concepts such as the core where no group of players can be better off by deviating from their current coalition.}
      \item Most models require full information on player preference
      \item Lack of large real-world dataset with ground truth
    \end{itemize}
...
\begin{frame}{Data}
  ...
  \begin{itemize}
    ...
    \item For the 20th Knesset: 147 parliament members' votes on over 7500 bills in $\sim$ 4 years
    \item Ground truth clusters: political party affiliations
    \begin{itemize}
      \item<2-> 10 parties aligned along a left-right axis
      \item<2-> Ideological agreement among the government (right) and the opposition (left) parties respectively
    \end{itemize}
  \end{itemize}
  \column{0.2\textwidth}
  \uncover<2->{
    \begin{figure}
      \includegraphics[width=0.9\linewidth]{parties}
    \end{figure}
  }
  \end{columns}
\end{frame}
...
\begin{frame}{Probably Approximately Correct (PAC) Stability}
  ...
  The \textit{local loss} of a coalition structure $\pi$ and a coalition $S \subseteq N$:
...
$$L_{\cal D}(\pi) = \Pr_{S \sim \cal D}\left[\lambda(\pi,S) =1\right]\label{eq:total-loss-D}$$
  \note{Core stability requires this probability to equal 0, meaning no group of players can be better off by deviating. }
  \note{This captures a probabilistic variant of the core condition: rather than requiring that $\lambda(\pi,S) = 0$ for all $S \subseteq N$ (as is the case for the core), we require that it is low, w.r.t. distribution $\cal D$.}
...
\begin{frame}{Methodology}{Game Theoretic Models}
  \begin{itemize}
    \item Coalition formation games with hedonic preferences
...
\small
\alert{Selective enemies} preference profile:
...
\alert{General enemies} preference profile:
...
\begin{frame}{Bottom Responsive Games}{Core Finding Algorithms - Full Information}
  \begin{algorithm}[H]
    ...
    \While {$S \neq \emptyset$}
      ...
      \While {$\Phi = \emptyset$}
        \State $\Gamma \leftarrow \underset{X \in \Gamma}{\bigcup} \underset{i \in X}{\bigcup} \{ X \setminus \{j\} | j \in Y \text{ for some } Y \in \Av(i, X)\}$
...
value function values a winning majority over
...
\begin{frame}{Quantitative Analysis}{SBM - Normal Edge Weights}
  \small
  \begin{columns}
  \column{0.5\textwidth}
  \begin{figure}
